...
- 지영희류 Hero는 하나의 desktop/mobile 배경을 전체 영역에 연속 배치한다. 3D 패키지는 기존 대비 desktop 약 10%, mobile 약 20% 작게 표시하고, mobile에서는 패키지 다음에 정보가 잘리지 않고 이어진다. 오른쪽 위 key light와 왼쪽 아래로 흐르는 실제 3D 그림자, 최대 8배 anisotropy·mipmap과 최대 2 DPR로 인쇄 디테일을 자연스럽게 보존하며 정보의 작은 라벨·상태·구분선·링크에만 옷고름 계열 blue accent를 사용한다.
